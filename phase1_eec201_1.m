%frame
[audio,Fs]=audioread('C:\Users\arind\Documents\winter_quarter_2024\eec201\final_project\Test_Data\s8.wav');
frame_length=0.02;
overlap_length=0.01;                                                                                                
% for i=1:size(audio,1)
%     audio1(i)=mean(audio(i,:));
% end
...
[frames1, f,t]=stft(pre_emphasized_audio,Fs,Window=hamming(frame_size),OverlapLength=frame_step,FFTLength=256);
ps=(abs(frames1).^2)./256;
...
% plot(frames(:,10));
% windowing
% hamming_window = kaiser(frame_size,2);
...
save('s8_test_1',"cepstrum_frames")
% figure
% for i=1:19
